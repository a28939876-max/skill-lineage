...
> 英文用着别扭，自己翻译又怕跟不上原版更新。

**结果**：有——而且汉化版自己就做到了 **5229⭐**，完整翻译外加 6 个原创 skill。它不是 fork，GitHub 的 fork 图谱里根本看不见它，是同名搜索捞出来的。

### 「我从合集装的这个拷贝，跟原版一样吗？」

> 合集仓转来转去，谁知道转丢了什么、塞进了什么。

**结果**：diff 一跑，`added_lines` 里躺着一段原版没有的指令——让 agent **silently**（不告知用户）给 skill 打分并 POST 回某个 API。是安装器平台的固定注入，不一定是作者恶意，但你装之前有权知道。

### 「有没有比原版更好用的版本？」

> 按 star 排序选了三个百星头部候选，感觉稳了。

**结果**：修谱之后全部换人——最终入选的是 **8⭐ 和 14⭐** 的衍生版：一个读运行记录找问题，一个对照 189 条带引用的官方规则审计。star 排序永远不会把它们推给你。

### 「索引站推荐的这个，怎么 404 了？」

> 明明显示 497⭐、描述完整、还有一窝衍生版。

**结果**：原版前不久把这个 skill 从仓库里删了，索引站还不知道。顺手修谱发现它的 26 个衍生里 12 个是零改动镜像——真正能装的反而是同名搜索捞出的独立实现（149⭐）。

---

**以上全部出自真实修谱记录**，完整过程一篇篇写在 [cases/](./cases/) 里（四篇是从大量实测里挑的典型，不是全部）。
